...
def process_image(image_name,iname):
    image = cv2.imread(image_name)

    image_blurred = cv2.medianBlur(image,15)
    #image_blurred = image
    ...
    cv2.imwrite(pathresult+iname+".jpg", image)
    return cv2.normalize(src=mask_img, dst=None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
